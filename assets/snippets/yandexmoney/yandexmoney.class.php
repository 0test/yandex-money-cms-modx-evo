<?php
if (!defined('YANDEXMONEY_VERSION')) {
    define('YANDEXMONEY_VERSION', '1.4.3');
}
    ...
    function YandexMoneyForm(&$fields)
    {
        ...
        if ($_POST) {
            ...
            if (isset($_POST['phone'])) {
                $ym->phone = $_POST['phone'];
            }
            if (isset($_POST['email'])) {
                $ym->email = $_POST['email'];
            }
...
if (!class_exists('Yandexmoney')) {
    class Yandexmoney
    {
        ...
        {
            return (in_array($this->pay_method,
                    array('PC', 'AC', 'MC', 'GP', 'WM', 'AB', 'SB', 'MA', 'PB', 'QW', 'CR')) || $this->paymode);
        }
        ...
        public function ProcessResult()
        {
            ...
            if ($this->checkSign($callbackParams)) {
                if ($this->org_mode) {
                    ...
                    $order_query = $modx->db->select("*", $modx->getFullTableName('manager_shopkeeper'),
                        'id = '.(int)$callbackParams["orderNumber"], "", "");
                    $order       = $modx->db->getRow($order_query, 'both');
                    $price       = floatval(str_replace(',', '.', $order['price']));
		   ...
		   <tr class="select-w">
            ...
            <td>
				...
				<div class="org" style="display:none;">
					...
					<input type="checkbox" name="config[method_cr]" value="1" id="ym_method_11" '.(($this->method_cr == 1 ? 'checked' : '')).'><label for="ym_method_11" style="width: 280px;text-align: left;">Заплатить по частям</label> <br>
				</div>
			 </td>
          </tr>
		 <tr  class="select-wo">
				...
				<table style="border: 1px black solid;">
				  <tbody><tr>
				   ...
				   <tr>
						...
						<td style="border: 1px black solid; padding: 5px;">'.str_replace("http://", "https://",
                    $site_url).'assets/snippets/yandexmoney/callback.php</td>
				   </tr>
				   <tr>
